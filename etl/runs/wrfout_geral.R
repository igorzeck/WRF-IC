...
### WRF - Visibilidade de Koschmieder
df_var <- tibble(datetime = c(seq_h))
for (dom in 1:4) {
  path_ = paste0(comp_info$prefix,"d0",dom, comp_info$suffix)
  nc_arq <- nc_open(path_)
  print(paste("Calculando vis_wrf para o domínio", dom))
  nc_var <- calc_wrf_vis(nc_arq, seq_h)
  
  # Normalização min-max (para teto de 1e4)
  if (sd(nc_var, na.rm = TRUE) != 0) {
    df_var <- df_var %>% 
      mutate("{paste0('d0',dom)}" :=  (nc_var - min(nc_var))/(max(nc_var) - min(nc_var)) * 1e4)
  } else {
    df_var <- df_var %>% 
      mutate("{paste0('d0',dom)}" :=  nc_var)
  }
  nc_close(nc_arq)
}
