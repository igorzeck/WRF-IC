# Script para o treino de um modelo LightGBM com METAR de jan + fev de 2026
# Periodo: 15/jan a 15/fev com domínio d03 para configurações padrão
# Dados de LMLT são do ERA5
...
dataset_path <- "datasets/df_treino_d03.csv"
...
## 3. Data Splitting ----
# Split 80/20
train_index <- createDataPartition(df_clean$vis, p = 0.8, list = FALSE)
train_data  <- df_clean[train_index, ]
test_data   <- df_clean[-train_index, ]

# O LightGBM exige matrizes numéricas para as features (X) e um vetor numérico para o target (y)
X_train <- as.matrix(train_data %>% select(-vis))
y_train <- train_data$vis
X_test  <- as.matrix(test_data %>% select(-vis))
...
## 6. Salva modelo ----
model_path <- "models/lightgbm_metar_jan_fev_2026.rds"
