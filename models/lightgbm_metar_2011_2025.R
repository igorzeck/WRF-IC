# Script para o treino de um modelo LightGBM com METAR de 2025
...
df_clean <- df_raw %>%
  ...
  select(-datetime) %>%
  mutate(
    clima = as.integer(as.factor(clima)),
    categ_nuvem = as.integer(as.factor(categ_nuvem))
  )
...
## 6. Salva modelo ----
model_path <- "models/lightgbm_metar_2011_2025.rds"
